...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "PATCH" && url.pathname.startsWith("/model/") && url.pathname.endsWith("/design")) { const name=decodeURIComponent(url.pathname.split("/")[2]),entity=entities.get(name),file=entityFile(name);if(!entity||!file)return send(res,404,{error:"Unknown entity"});const data=await body(req),features=data.architectureFeatures;if(!features||typeof features!=="object"||["auditStamp","trackHistory"].some(key=>typeof features[key]!=="boolean"))return send(res,400,{error:"auditStamp and trackHistory must be boolean"});entity.architectureFeatures={auditStamp:features.auditStamp,trackHistory:features.trackHistory};fs.writeFileSync(file,JSON.stringify(entity,null,2));return send(res,200,entity); }
